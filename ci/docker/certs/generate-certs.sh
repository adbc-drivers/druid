...
if [ -f "${CA_CERT}" ] && [ -f "${KEYSTORE}" ] && [ "${DRUID_FORCE_REGENERATE_CERTS:-0}" != "1" ]; then
  echo "Druid TLS assets already exist"
  exit 0
fi

TMP_DIR=$(mktemp -d "${TMPDIR:-/tmp}/druid-certs.XXXXXX")
cleanup() {
  rm -rf "${TMP_DIR}"
}
